...
<select id="dispositivo" name="codigo" >
    <?php
    //while($f=mysql_fetch_array($re)){
      //  echo'<option value='.$f[rut_cliente].'>'.$f[nombreempresa].'</option>';
        //}
    ?>
</select>-->
<!--
...
<h5>Ruta</h5>
    <?php
    $re= mysql_query("select * from ruta");
    //el @ sirve para que no salga ese mensaje de NOTICE al momento de ejecutar por 1ra ve nuestro codigo.
   ?>
<select id="dispositivo" name="ruta" required="required">
    <?php
    while($f=mysql_fetch_array($re)){
...
<!--<h5>Fecha </h5>
    <input type="date" name="fecha" style="margin-left:0px;" id="fechaini" class="form-emp" size="30" maxlength="1000" required="required"/>
-->
 <?php //$fechaini= fechaini?>
    <!--
  <h5>Hora Inicial</h5>
  <input type="time" name="tiempoini" style="margin-left:0px;" id="tiempoini" class="form-emp" size="30" maxlength="1000" /> 
 
  <h5>Hora Final</h5>
  <input type="time" name="tiempofin" style="margin-left:0px;" id="fechafin" class="form-emp" size="30" maxlength="1000" />
-->
